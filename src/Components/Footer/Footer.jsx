...
import PapadolaLogo from '../../assets/Images/PapadolaLogo.jpg'
...
const Footer = () => {
  return (
    ...
    <div className="w-full max-w-6xl mx-auto px-5">
      ...
      <img src={PapadolaLogo} width={50} height={50} alt="Logo" />
      <span className="text-3xl font-semibold">Papadola</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <ul className="space-y-4">
            <li className="flex items-center space-x-2">
                <FaPhoneAlt className="text-3xl text-mainColor bg-white rounded-full p-2"  />
                 <span>Phone: 034851500</span>
               </li>
               <li className="flex items-center space-x-2">
                 <FaWhatsapp className="text-3xl text-mainColor bg-white rounded-full p-2"  />
                 <span>WhatsApp: +201203888826</span>
              </li>
          </ul>
          {/* Social Media Links */}
          <div className="flex space-x-4 mt-4">
            <Link
              to="https://www.facebook.com/papadola.restaurant"
              target="_blank"
            ...
            <Link
              to="https://www.instagram.com/papadola.restaurant/?hl=ar"
              target="_blank"
              className="p-3 bg-white text-mainColor rounded-full hover:bg-gray-300 transition duration-300"
            >
        ...
        {["Home", "Menu", "Contact Us"].map((page, index) => (  
          <li key={index}>
            <NavLink
              to={page === "Menu" ? "papadola_menu" : page.toLowerCase().replace(" ", "_")}
              className={({ isActive }) =>
      ...
      <Link to="https://food2go.online/" target="_blank" className="flex items-center justify-center gap-2 text-sm mt-2 border-t border-white pt-4">
        ...
        <h1 className="text-white font-semibold text-lg">Papadola</h1>
        </Link>
    </div>
  </div>
  );
